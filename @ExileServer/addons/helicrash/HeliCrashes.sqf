...
// Number of Crashes
_crashNum = 10;
_centerPOS = [7680,7852.8,0]; //[10088,9045,0]; original  [6145,6010,0]; esseker
_minDistCenter = 0; // minimum distance from the center position (Number) in meters
_maxDist = 7000; // maximum distance from the center position (Number) in meters
_minDist = 5; // minimum distance from the nearest object (Number) in meters, ie. spawn at least this distance away from anything within x meters. Low _mindist means helicrashes could spawn near towns or in forests.. higher the number it would be spawning in an open field etc
